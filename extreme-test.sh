...
run_test "Extreme 4.1 - token-check (no header)"     "$exp_protected" "$BASE_URL/token-check"
run_test "Extreme 4.1 - token-binding-1 (no header)" "$exp_protected" "$BASE_URL/token-binding-1"
run_test "Extreme 4.1 - token-binding-2 (no header)" "$exp_protected" "$BASE_URL/token-binding-2"

# 4.2 Valid token header only (no binding headers)
if [ -f "$TOKDIR/approov_token_1_valid" ]; then
  run_test "Extreme 4.2 - unprotected (+valid token only)" 200 \
    -H "$HDR_NAME: $(cat "$TOKDIR/approov_token_1_valid")" \
    "$BASE_URL/unprotected"

  run_test "Extreme 4.2 - token-check (+valid token only)" 200 \
    -H "$HDR_NAME: $(cat "$TOKDIR/approov_token_1_valid")" \
    "$BASE_URL/token-check"

  run_test "Extreme 4.2 - token-binding-1 (+valid token only)" "$exp_protected" \
    -H "$HDR_NAME: $(cat "$TOKDIR/approov_token_1_valid")" \
    "$BASE_URL/token-binding-1"

  run_test "Extreme 4.2 - token-binding-2 (+valid token only)" "$exp_protected" \
    -H "$HDR_NAME: $(cat "$TOKDIR/approov_token_1_valid")" \
    "$BASE_URL/token-binding-2"
else
  skip_test "Extreme 4.2 (valid token only)"
fi

# 4.3 Valid token + valid binding headers

# binding-1 + token-check + unprotected -> use token bound to Authorization only
if [ -f "$TOKDIR/approov_token_2_valid" ]; then
  run_test "Extreme 4.3 - token-binding-1 (+valid token + Authorization)" 200 \
    -H "Authorization: $AUTH_VAL" \
    -H "$HDR_NAME: $(cat "$TOKDIR/approov_token_2_valid")" \
    "$BASE_URL/token-binding-1"

  run_test "Extreme 4.3 - unprotected (+valid token + Authorization)" 200 \
    -H "Authorization: $AUTH_VAL" \
    -H "$HDR_NAME: $(cat "$TOKDIR/approov_token_2_valid")" \
    "$BASE_URL/unprotected"

  run_test "Extreme 4.3 - token-check (+valid token + Authorization)" 200 \
    -H "Authorization: $AUTH_VAL" \
    -H "$HDR_NAME: $(cat "$TOKDIR/approov_token_2_valid")" \
    "$BASE_URL/token-check"
  ...
  skip_test "Extreme 4.3 (binding-1/token-check/unprotected/token-binding-2)"
fi

# 4.4 Valid token + Authorization + Content-Digest
  if [ -f "$TOKDIR/approov_token_3_valid" ]; then
    run_test "Extreme 4.4 - token-binding-2 (+valid token + Authorization + Content-Digest)" 200 \
    -H "Authorization: $AUTH_VAL2" \
    -H "Content-Digest: $CD_VAL" \
    -H "$HDR_NAME: $(cat "$TOKDIR/approov_token_3_valid")" \
    "$BASE_URL/token-binding-2"

    run_test "Extreme 4.4 - unprotected (+valid token + Authorization + Content-Digest)" 200 \
    -H "Authorization: $AUTH_VAL2" \
    -H "Content-Digest: $CD_VAL" \
    -H "$HDR_NAME: $(cat "$TOKDIR/approov_token_3_valid")" \
    "$BASE_URL/unprotected"

    run_test "Extreme 4.4 - token-check (+valid token + Authorization + Content-Digest)" 200 \
    -H "Authorization: $AUTH_VAL2" \
    -H "Content-Digest: $CD_VAL" \
    -H "$HDR_NAME: $(cat "$TOKDIR/approov_token_3_valid")" \
    "$BASE_URL/token-check"
